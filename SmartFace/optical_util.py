...
def propagator(plane_size = 512, grid = 1, propagate_distance = 500, wavelength = 1, G_norm = 1, shift = (0, 0)):
    '''
    ...
    print("G_shift: ", shift)
    x = np.arange(-(plane_size-1), plane_size,1) * grid - shift[0]
    y = np.arange(-(plane_size-1), plane_size,1) * grid - shift[1]
